...
struct create_sub_geometry : boost::static_visitor<int> {
  ...
  create_sub_geometry(geom_type_t type, boostgeom_writer_t *writer)
      : type(type)
      , writer(writer) {}

  int operator()(boost::blank &blank) const { return SQLITE_IOERR; }

  int operator()(void *geom) const { return SQLITE_IOERR; }
  ...
  template <typename Geometry> static Geometry *add(GeometryCollection *geom) {
    geom->emplace_back();
    geom->back() = Geometry();
...
static int boostgeom_begin_geometry(const struct geom_consumer_t *consumer, const geom_header_t *header,
                                    errorstream_t *error) {
  ...
  if (writer->offset == 0) {
    ...
    int result = boost::apply_visitor(create_sub_geometry(header->geom_type, writer),
                                      writer->geometry_stack[writer->offset - 1]);
...
struct assign_coordinates : boost::static_visitor<int> {
  ...
  int operator()(boost::blank &geom) const { return SQLITE_IOERR; }

  int operator()(void *geom) const { return SQLITE_IOERR; }

  int operator()(Point *geom) const {
    boost::geometry::set<0>(*geom, coords[0]);
    boost::geometry::set<1>(*geom, coords[1]);
    return SQLITE_OK;
  }
...
static int boostgeom_coordinates(const struct geom_consumer_t *consumer, const geom_header_t *header,
                                 size_t point_count, const double *coords, int skip_coords, errorstream_t *error) {
  boostgeom_writer_t *writer = (boostgeom_writer_t *)consumer;
  return boost::apply_visitor(assign_coordinates(header, point_count, coords), writer->geometry_stack[writer->offset]);
}

static int boostgeom_end_geometry(const struct geom_consumer_t *consumer, const geom_header_t *header,
                                  errorstream_t *error) {
...
geom_consumer_t *boostgeom_writer_geom_consumer(boostgeom_writer_t *writer) { return &writer->geom_consumer; }

GeometryPtr boostgeom_writer_getgeometry(boostgeom_writer_t *writer) { return writer->geometry; }

#define COORD_BATCH_SIZE 10

template <typename Range> struct boostgeom_range {
  static inline int apply(Range const &range, geom_consumer_t const *consumer, const geom_header_t *header,
                          errorstream_t *error) {
    ...
    while (remaining > 0) {
      ...
      for (size_t i = 0, ix = 0; i < points_to_read; i++, ix += 2) {
        ...
        coord[ix] = boost::geometry::get<0>(pt);
        coord[ix + 1] = boost::geometry::get<1>(pt);
        ++it;
      }
...
static int read_boostgeom_point(const Point &geom, geom_consumer_t const *consumer, errorstream_t *error) {
  int result = SQLITE_OK;
  geom_header_t header = {GEOM_POINT, GEOM_XY, 2};

  if (result == SQLITE_OK) {
    result = consumer->begin_geometry(consumer, &header, error);
  }
  if (result == SQLITE_OK) {
    double coord[] = {boost::geometry::get<0>(geom), boost::geometry::get<1>(geom)};
...
static int read_boostgeom_linestring(const LineString &geom, geom_consumer_t const *consumer, errorstream_t *error) {
  int result = SQLITE_OK;
  geom_header_t header = {GEOM_LINESTRING, GEOM_XY, 2};
...
static int read_boostgeom_linearring(const LinearRing &geom, geom_consumer_t const *consumer, errorstream_t *error) {
  int result = SQLITE_OK;
  geom_header_t header = {GEOM_LINEARRING, GEOM_XY, 2};
...
static int read_boostgeom_polygon(const Polygon &geom, geom_consumer_t const *consumer, errorstream_t *error) {
  int result = SQLITE_OK;
  geom_header_t header = {GEOM_POLYGON, GEOM_XY, 2};
  ...
  if (result == SQLITE_OK && (exterior.size() > 0 || std::any_of(interior.cbegin(), interior.cend(),
                                                                 [](const LinearRing &lr) { return lr.size() > 0; }))) {
    result = read_boostgeom_linearring(exterior, consumer, error);

    std::vector<LinearRing> const &interior_rings =
        static_cast<std::vector<LinearRing> const &>(boost::geometry::interior_rings(geom));
...
static int read_boostgeom_multipoint(const MultiPoint &geom, geom_consumer_t const *consumer, errorstream_t *error) {
  int result = SQLITE_OK;
  geom_header_t header = {GEOM_MULTIPOINT, GEOM_XY, 2};
...
static int read_boostgeom_multilinestring(const MultiLineString &geom, geom_consumer_t const *consumer,
                                          errorstream_t *error) {
  int result = SQLITE_OK;
  geom_header_t header = {GEOM_MULTILINESTRING, GEOM_XY, 2};
...
static int read_boostgeom_multipolygon(const MultiPolygon &geom, geom_consumer_t const *consumer,
                                       errorstream_t *error) {
  int result = SQLITE_OK;
  geom_header_t header = {GEOM_MULTIPOLYGON, GEOM_XY, 2};
...
static int read_boostgeom_geometrycollection(const GeometryCollection &geom, geom_consumer_t const *consumer,
                                             errorstream_t *error) {
  int result = SQLITE_OK;
  geom_header_t header = {GEOM_GEOMETRYCOLLECTION, GEOM_XY, 2};
...
static int read_boostgeom_box(const Envelope &geom, geom_consumer_t const *consumer, errorstream_t *error) {
  int result = SQLITE_OK;
  geom_header_t header = {GEOM_MULTIPOINT, GEOM_XY, 2};
  ...
  if (result == SQLITE_OK) {
    result = read_boostgeom_point(geom.min_corner(), consumer, error);
  }

  if (result == SQLITE_OK) {
    result = read_boostgeom_point(geom.max_corner(), consumer, error);
  }
...
struct read_geometry : boost::static_visitor<int> {
  ...
  read_geometry(geom_consumer_t const *consumer, errorstream_t *error)
      : consumer(consumer)
      , error(error) {}

  int operator()(const Point &geom) const { return read_boostgeom_point(geom, consumer, error); }

  int operator()(const Point *geom) const { return read_boostgeom_point(*geom, consumer, error); }

  int operator()(const LineString &geom) const { return read_boostgeom_linestring(geom, consumer, error); }

  int operator()(const LineString *geom) const { return read_boostgeom_linestring(*geom, consumer, error); }

  int operator()(const LinearRing &geom) const { return read_boostgeom_linearring(geom, consumer, error); }

  int operator()(const LinearRing *geom) const { return read_boostgeom_linearring(*geom, consumer, error); }

  int operator()(const Polygon &geom) const { return read_boostgeom_polygon(geom, consumer, error); }

  int operator()(const Polygon *geom) const { return read_boostgeom_polygon(*geom, consumer, error); }

  int operator()(const MultiPoint &geom) const { return read_boostgeom_multipoint(geom, consumer, error); }

  int operator()(const MultiPoint *geom) const { return read_boostgeom_multipoint(*geom, consumer, error); }

  int operator()(const MultiLineString &geom) const { return read_boostgeom_multilinestring(geom, consumer, error); }

  int operator()(const MultiLineString *geom) const { return read_boostgeom_multilinestring(*geom, consumer, error); }

  int operator()(const MultiPolygon &geom) const { return read_boostgeom_multipolygon(geom, consumer, error); }
  ...
  int operator()(const boost::blank &geom) const { return SQLITE_IOERR; }

  int operator()(const Envelope &geom) const { return read_boostgeom_box(geom, consumer, error); }
