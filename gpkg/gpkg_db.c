...
static column_info_t gpkg_spatial_ref_sys_columns[] = {{"srs_name", "TEXT", N, SQL_NOT_NULL, NULL},
...
static value_t gpkg_spatial_ref_sys_data[] = {
    T("Undefined Cartesian"),
    I(-1),
    T("NONE"),
    I(-1),
    T("undefined"),
    N,
    T("Undefined Geographic"),
    I(0),
    T("NONE"),
    I(0),
    T("undefined"),
    ...
    T("EPSG"),
    I(4326),
    T("GEOGCS[\"WGS 84\", DATUM[\"WGS_1984\", SPHEROID[\"WGS 84\",6378137,298.257223563, "
    ...
    N,
    T("WGS 84 / Pseudo-Mercator"),
    I(3857),
    T("EPSG"),
    I(3857),
    T("GEOGCS[\"WGS 84\",DATUM[\"WGS_1984\",SPHEROID[\"WGS "
      "84\",6378137,298.257223563,AUTHORITY[\"EPSG\",\"7030\"]],AUTHORITY[\"EPSG\",\"6326\"]],PRIMEM[\"Greenwich\",0,"
      "AUTHORITY[\"EPSG\",\"8901\"]],UNIT[\"degree\",0.0174532925199433,AUTHORITY[\"EPSG\",\"9122\"]],AUTHORITY["
      "\"EPSG\",\"4326\"]]"),
    N};
static table_info_t gpkg_spatial_ref_sys = {"gpkg_spatial_ref_sys", gpkg_spatial_ref_sys_columns,
                                            gpkg_spatial_ref_sys_data, 4};
...
static column_info_t gpkg_contents_columns[] = {
    ...
    {"srs_id", "INTEGER", N, 0,
     "CONSTRAINT fk_srid__gpkg_spatial_ref_sys_srs_id REFERENCES gpkg_spatial_ref_sys(srs_id)"},
    {NULL, NULL, N, 0, NULL}};
static table_info_t gpkg_contents = {"gpkg_contents", gpkg_contents_columns, NULL, 0};

static column_info_t gpkg_extensions_columns[] = {{"table_name", "TEXT", N, SQL_UNIQUE(1), NULL},
                                                  {"column_name", "TEXT", N, SQL_UNIQUE(1), NULL},
                                                  {"extension_name", "TEXT", N, SQL_NOT_NULL | SQL_UNIQUE(1), NULL},
                                                  {"definition", "TEXT", N, SQL_NOT_NULL, NULL},
                                                  {"scope", "TEXT", N, SQL_NOT_NULL, NULL},
                                                  {NULL, NULL, N, 0, NULL}};
static table_info_t gpkg_extensions = {"gpkg_extensions", gpkg_extensions_columns, NULL, 0};

static column_info_t gpkg_geometry_columns_columns[] = {
    {"table_name", "TEXT", N, SQL_NOT_NULL | SQL_PRIMARY_KEY | SQL_UNIQUE(1),
     "CONSTRAINT fk_table_name__gpkg_contents_table_name REFERENCES gpkg_contents(table_name)"},
    {"column_name", "TEXT", N, SQL_NOT_NULL | SQL_PRIMARY_KEY, NULL},
    {"geometry_type_name", "TEXT", N, SQL_NOT_NULL, NULL},
    {"srs_id", "INTEGER", N, SQL_NOT_NULL,
     "CONSTRAINT fk_srs_id__gpkg_spatial_ref_sys_srs_id REFERENCES gpkg_spatial_ref_sys(srs_id)"},
    {"z", "TINYINT", N, SQL_NOT_NULL, NULL},
    {"m", "TINYINT", N, SQL_NOT_NULL, NULL},
    {NULL, NULL, N, 0, NULL}};
static table_info_t gpkg_geometry_columns = {"gpkg_geometry_columns", gpkg_geometry_columns_columns, NULL, 0};

static column_info_t gpkg_tile_matrix_set_columns[] = {
    {"table_name", "TEXT", N, SQL_NOT_NULL | SQL_PRIMARY_KEY,
     "CONSTRAINT fk_table_name__gpkg_contents_table_name REFERENCES gpkg_contents(table_name)"},
    {"srs_id", "INTEGER", N, SQL_NOT_NULL,
     "CONSTRAINT fk_srs_id__gpkg_spatial_ref_sys_srs_id REFERENCES gpkg_spatial_ref_sys(srs_id)"},
    ...
    {NULL, NULL, N, 0, NULL}};
static table_info_t gpkg_tile_matrix_set = {"gpkg_tile_matrix_set", gpkg_tile_matrix_set_columns, NULL, 0};
...
static column_info_t gpkg_tile_matrix_columns[] = {
    ...
    {NULL, NULL, N, 0, NULL}};
static table_info_t gpkg_tile_matrix = {"gpkg_tile_matrix", gpkg_tile_matrix_columns, NULL, 0};
...
static column_info_t gpkg_data_columns_columns[] = {
    ...
    {NULL, NULL, N, 0, NULL}};
static table_info_t gpkg_data_columns = {"gpkg_data_columns", gpkg_data_columns_columns, NULL, 0};
...
static column_info_t gpkg_data_column_constraints_columns[] = {
    ...
    {"description", "TEXT", N, 0, NULL},
    {NULL, NULL, N, 0, NULL}};
static table_info_t gpkg_data_column_constraints = {"gpkg_data_column_constraints",
                                                    gpkg_data_column_constraints_columns, NULL, 0};
...
static column_info_t gpkg_metadata_columns[] = {
    ...
    {NULL, NULL, N, 0, NULL}};
static table_info_t gpkg_metadata = {"gpkg_metadata", gpkg_metadata_columns, NULL, 0};
...
static column_info_t gpkg_metadata_reference_columns[] = {
    ...
    {NULL, NULL, N, 0, NULL}};
static table_info_t gpkg_metadata_reference = {"gpkg_metadata_reference", gpkg_metadata_reference_columns, NULL, 0};

static const table_info_t *const gpkg_tables[] = {&gpkg_contents,
                                                  &gpkg_extensions,
...
static int gpkg_contents_geometry_table_check(sqlite3 *db, const char *db_name, errorstream_t *error) {
  int result = sql_exec_stmt(db, gpkg_contents_geometry_table_check_row, NULL, error,
                             "SELECT table_name FROM \"%w\".gpkg_contents WHERE data_type='features' AND table_name "
                             "NOT IN (SELECT table_name FROM \"%w\".gpkg_geometry_columns)",
                             db_name, db_name);
...
static int gpkg_contents_tilemetadata_table_check(sqlite3 *db, const char *db_name, errorstream_t *error) {
  int result = sql_exec_stmt(db, gpkg_contents_tilemetadata_table_check_row, NULL, error,
                             "SELECT table_name FROM \"%w\".gpkg_contents WHERE data_type='tiles' AND table_name NOT "
                             "IN (SELECT table_name FROM \"%w\".gpkg_tile_matrix_set)",
                             db_name, db_name);
...
static int gpkg_table_column_check(sqlite3 *db, const char *db_name, const char *table_name, const char *table_column,
                                   const char *column_column, errorstream_t *error) {
  int result = SQLITE_OK;
  ...
  if (column_column != NULL) {
    result = sql_exec_stmt(db, gpkg_table_column_check_row, NULL, &c, "SELECT \"%w\", \"%w\" FROM \"%w\".\"%w\"",
                           table_column, column_column, db_name, table_name);
  } else {
    result = sql_exec_stmt(db, gpkg_table_column_check_row, NULL, &c, "SELECT \"%w\", NULL FROM \"%w\".\"%w\"",
                           table_column, db_name, table_name);
  }
...
static check_func checks[] = {gpkg_contents_columns_table_column_check,   gpkg_extensions_table_column_check,
                              gpkg_geometry_columns_table_column_check,   gpkg_tile_matrix_table_column_check,
                              gpkg_tile_matrix_set_table_column_check,    gpkg_data_columns_table_column_check,
                              gpkg_metadata_reference_table_column_check, gpkg_contents_geometry_table_check,
                              gpkg_contents_tilemetadata_table_check,     NULL};
...
static int check(sqlite3 *db, const char *db_name, int flags, errorstream_t *error) {
  ...
  if (result == SQLITE_OK) {
    int features = 0;
    result = sql_exec_for_int(db, &features,
                              "SELECT count(*) FROM \"%w\".gpkg_contents WHERE data_type LIKE 'features'", db_name);
    if (result == SQLITE_OK) {
  ...
  if (result == SQLITE_OK) {
    int tiles = 0;
    result =
        sql_exec_for_int(db, &tiles, "SELECT count(*) FROM \"%w\".gpkg_contents WHERE data_type LIKE 'tiles'", db_name);
    if (result == SQLITE_OK) {
...
static int gpkg_writer_init(geom_blob_writer_t *writer) { return gpb_writer_init(writer, -1); }

static int add_geometry_column(sqlite3 *db, const char *db_name, const char *table_name, const char *column_name,
  ...
  result = sql_exec(db,
                    "INSERT INTO \"%w\".\"%w\" (table_name, column_name, geometry_type_name, srs_id, z, m) VALUES (%Q, "
                    "%Q, %Q, %d, %d, %d)",
...
static int create_tiles_table(sqlite3 *db, const char *db_name, const char *table_name, errorstream_t *error) {
  ...
  table_info_t tile_table_info = {table_name, gpkg_tiles_table_columns, NULL, 0};
...
static int create_spatial_index(sqlite3 *db, const char *db_name, const char *table_name,
  ...
  if (result != SQLITE_OK) {
    error_append(error, "Could not check if index table %s.%s exists: %s", db_name, index_table_name,
                 sqlite3_errmsg(db));
    goto exit;
  }
  ...
  result = sql_exec_for_int(
      db, &geom_col_count,
      "SELECT count(*) FROM \"%w\".gpkg_geometry_columns WHERE table_name LIKE %Q AND column_name LIKE %Q", db_name,
      table_name, geometry_column_name);
  if (result != SQLITE_OK) {
    error_append(error, "Could not check if column %s.%s.%s exists in %s.gpkg_geometry_columns: %s", db_name,
                 table_name, geometry_column_name, db_name, sqlite3_errmsg(db));
    goto exit;
  }

  if (geom_col_count == 0) {
    error_append(error, "Column %s.%s.%s is not registered in %s.gpkg_geometry_columns", db_name, table_name,
                 geometry_column_name, db_name);
    goto exit;
  }

  result = sql_exec(db, "CREATE VIRTUAL TABLE \"%w\".\"%w\" USING rtree(id, minx, maxx, miny, maxy)", db_name,
                    index_table_name);
  if (result != SQLITE_OK) {
  ...
  result = sql_exec(db,
                    ...
                    index_table_name, id_column_name, geometry_column_name, geometry_column_name, geometry_column_name,
  ...
  result = sql_exec(db,
                    ...
                    "  );\n"
                    "END;",
                    db_name, table_name, geometry_column_name, geometry_column_name, table_name, id_column_name,
                    id_column_name, geometry_column_name, geometry_column_name, index_table_name, id_column_name,
                    geometry_column_name, geometry_column_name, geometry_column_name, geometry_column_name);
  if (result != SQLITE_OK) {
  ...
  result =
      sql_exec(db,
               ...
               "  );\n"
               "END;",
               db_name, table_name, geometry_column_name, table_name, id_column_name, id_column_name,
               geometry_column_name, geometry_column_name, index_table_name, id_column_name, index_table_name,
               id_column_name, geometry_column_name, geometry_column_name, geometry_column_name, geometry_column_name);
  if (result != SQLITE_OK) {
  ...
  result = sql_exec(db,
                    ...
                    "END;",
                    db_name, table_name, geometry_column_name, table_name, id_column_name, id_column_name,
                    geometry_column_name, geometry_column_name, index_table_name, id_column_name, id_column_name);
  if (result != SQLITE_OK) {
  ...
  result = sql_exec(db,
                    ...
                    "  DELETE FROM \"%w\" WHERE id = OLD.\"%w\";\n"
                    "END;",
                    db_name, table_name, geometry_column_name, table_name, index_table_name, id_column_name);
  if (result != SQLITE_OK) {
  ...
  result = sql_exec(
      ...
      db_name, index_table_name, id_column_name, geometry_column_name, geometry_column_name, geometry_column_name,
      geometry_column_name, db_name, table_name, geometry_column_name, geometry_column_name);
  if (result != SQLITE_OK) {
...
static const spatialdb_t GEOPACKAGE = {"GeoPackage",
                                       NULL,
                                       init,
                                       check,
                                       write_blob_header,
...
const spatialdb_t *spatialdb_geopackage_schema() { return &GEOPACKAGE; }
