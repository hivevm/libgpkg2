...
# Vendored third-party trees are not held to this repository's documentation rules. icu/src is a
# git submodule that CI checks out empty; excluding it keeps local runs consistent with CI.
VENDORED=(-not -path "$ROOT/icu/src/*" -not -path "$ROOT/boost/libs/*" -not -path "$ROOT/build/*")
...
check_relative_links() {
  ...
  done < <(find "$ROOT" -type f -name '*.md' -not -path '*/.git/*' "${VENDORED[@]}" | sort)
}
...
check_section_refs() {
  ...
  done < <(find "$ROOT" -type f \( -name '*.md' -o -name '*.yml' -o -name '*.yaml' -o -name '*.sh' \) -not -path '*/.git/*' "${VENDORED[@]}" | sort)
}
